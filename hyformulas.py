...
#		query = "(way['golf'](" + coord_string + ");way['natural'='wood'](" + coord_string + ");node['natural'='tree'](" + coord_string + ");way['landuse'='forest'](" + coord_string + ");way['natural'='water'](" + coord_string + "););out;"
		...
		query = "(way['golf'](" + coord_string + ");way['natural'='wood'](" + coord_string + ");node['natural'='tree'](" + coord_string + ");way['landuse'='forest'](" + coord_string + ");way['natural'='water'](" + coord_string + ");way['waterway'='stream'](" + coord_string + ");way['waterway'='river'](" + coord_string + ");way['waterway'='ditch'](" + coord_string + "););out;"
...
def categorizeWays(hole_result, hole_minlat, hole_minlon, hole_maxlat, hole_maxlon, x_dim, y_dim):
	...
	for way in hole_result.ways:
		...
		elif waterway_type == "stream" or waterway_type == "river":
			streams.append(translateWaytoNP(way, hole_minlat, hole_minlon, hole_maxlat, hole_maxlon, x_dim, y_dim))

		elif waterway_type == "ditch":
			ditches.append(translateWaytoNP(way, hole_minlat, hole_minlon, hole_maxlat, hole_maxlon, x_dim, y_dim))

		elif highway_type == "path":
			paths.append(translateWaytoNP(way, hole_minlat, hole_minlon, hole_maxlat, hole_maxlon, x_dim, y_dim))
		else:
			continue
	...
	return sand_traps, tee_boxes, fairways, water_hazards, woods, trees, streams, ditches, paths
...
def drawFeature(image, array, color, line, feature_type):
    nds = np.array(array)
    print("node 0", nds[0])

    if feature_type == "tee_boxes":  # Detect squares and avoid spline smoothing
        smooth_points = np.array(nds)  # Use original points
    else:
        tck, u = splprep([nds[:, 0], nds[:, 1]], s=0, per=1)
        smooth_lat, smooth_lon = splev(np.linspace(0, 1, 100000), tck)
        smooth_points = np.vstack((smooth_lat, smooth_lon)).T
        smooth_points = np.round(smooth_points).astype(int)

    smooth_points = smooth_points.reshape((-1, 1, 2)).astype(np.int32)
...
# for a list of arrays and an image, draw each array as a line on the image (in a given color)
def drawLines(image, feature_list, color, line):
    for feature_nodes in feature_list:
        nds = np.array(feature_nodes).astype(np.int32)
        cv2.polylines(image, [nds], False, color, line, lineType=cv2.LINE_AA)

# for a list of arrays and an image, draw each array as a polygon on the image (in a given color)
def drawFeatures(image, feature_list, color, line, feature_type=None):
	for feature_nodes in feature_list:
		drawFeature(image, feature_nodes, color, line, feature_type)
...
def drawCarry(image, green_center, carrypoint, tee_box_points, ypp, text_size, text_color, right):
	...
	if maxpoint_distance < 100 or maxpoint_distance > 725:
...
#	if dtg < 40 or maxpoint_distance < 215 or maxpoint_distance > 290:
	if dtg < 40 or maxpoint_distance < 100 or maxpoint_distance > 790:
...
# eddited text_weight removing *2 after size)
	text_weight = round(text_size*2)
	...
	for point in distance_points:
		...
		if right:
			...
			(label_width, label_height), baseline = cv2.getTextSize(str(distance), cv2.FONT_HERSHEY_SIMPLEX, text_size, text_weight)
# changed 75 to 25 below right hand line maybe
		#	drawpoint = (point[0] - 30 - label_width,point[1])
			drawpoint = (point[0],point[1])

		else:
# changed 75 to 25 below for left hand lines next to trees
		#	drawpoint = (point[0] + 30,point[1])
			drawpoint = (point[0],point[1])

# removing the lines from the trees
	#	cv2.line(image,(int(point[0]),int(point[1])),(int(drawpoint[0]),int(drawpoint[1])),text_color, 3)
# removed *2 from size
		text_weight = round(text_size*2)

		(label_width, label_height), baseline = cv2.getTextSize(str(distance), cv2.FONT_HERSHEY_SIMPLEX,text_size*2, text_weight)
# editted
		x = int(drawpoint[0] + 2*text_weight)
#		x = int(drawpoint[0] - 2 * text_weight - label_width) # move text to the left of the line
		y = int(drawpoint[1] + 0.5*label_height)

		cv2.putText(image, str(distance), (x,y), cv2.FONT_HERSHEY_SIMPLEX, text_size*1.5, text_color, text_weight)
...
def generateYardageBook(latmin,lonmin,latmax,lonmax,replace_existing,colors,chosen_tbox,filter_width=50,short_factor=1,med_factor=1):
	...
	for way in ways:
		...
		sand_traps, tee_boxes, fairways, water_hazards, woods, trees, streams, ditches, paths = categorizeWays(hole_result, hole_minlat, hole_minlon, hole_maxlat, hole_maxlon, x_dim, y_dim)
		...
		rotated_streams = rotateArrayList(image, streams, angle)
		rotated_ditches = rotateArrayList(image, ditches, angle)
		rotated_paths = rotateArrayList(image, paths, angle)
		...
		filtered_water_hazards = filterArrayList(rotated_waypoints, rotated_water_hazards, ypp, hole_par, filter_yards=filter_width)
		...
		filtered_woods = filterArrayList(rotated_waypoints, rotated_woods, ypp, hole_par, filter_yards=filter_width)
		filtered_trees = filterArrayList(rotated_waypoints, rotated_trees, ypp, hole_par, filter_yards=filter_width)
		filtered_streams = filterArrayList(rotated_waypoints, rotated_streams, ypp, hole_par, filter_yards=filter_width)
		filtered_ditches = filterArrayList(rotated_waypoints, rotated_ditches, ypp, hole_par, filter_yards=filter_width)
		filtered_paths = filterArrayList(rotated_waypoints, rotated_paths, ypp, hole_par, filter_yards=filter_width)
		...
		final_streams, n1, n2, n3, n4 = adjustRotatedFeatures(filtered_streams, ymin, xmin)
		final_ditches, n1, n2, n3, n4 = adjustRotatedFeatures(filtered_ditches, ymin, xmin)
		final_paths, n1, n2, n3, n4 = adjustRotatedFeatures(filtered_paths, ymin, xmin)
		...
		drawFeatures(rotated_image, final_fairways, colors["fairways"], line=-1, feature_type="fairways")
		drawFeatures(rotated_image, final_tee_boxes, colors["tee boxes"], line=-1, feature_type="tee_boxes")  # Add feature type
		drawFeatures(rotated_image, final_water_hazards, colors["water"], line=-1, feature_type="water_hazards")
		drawFeatures(rotated_image, final_woods, colors["woods"], line=-1, feature_type="woods")
		drawFeatures(rotated_image, final_green_array, colors["greens"], line=-1, feature_type="greens")
		drawFeatures(rotated_image, final_sand_traps, colors["sand"], line=-1, feature_type="sand_traps")
		drawLines(rotated_image, final_streams, colors["water"], line=10)
		drawLines(rotated_image, final_ditches, colors["ditches"], line=20)
		drawLines(rotated_image, final_paths, colors["paths"], line=10)
		...
		# adjusted to 1.0/3000 from 1.5/3000

		text_size = 1.2/3000*eventual_height
...
# REmoved below to make red text vs black

#			drawGreenDistancesMin(rotated_image, adjusted_hole_array, final_tee_boxes, ypp, text_size*2, colors["text"], par_3_tees=1)
		...
		# for longer holes, there's more to do:
			...
			# draw carry distances to ditches
			add_r, add_l = drawCarryDistances(rotated_image, adjusted_hole_array, final_tee_boxes, final_ditches, ypp, text_size, colors["text"])

			right_carries += add_r
			left_carries += add_l

			add_r, add_l = drawCarryDistances(rotated_image, adjusted_hole_array, final_tee_boxes, final_streams, ypp, text_size, colors["text"])
			right_carries += add_r
			left_carries += add_l
			...
			drawGreenDistancesMin(rotated_image, adjusted_hole_array, final_ditches, ypp, text_size, colors["text"])  # Add this line
			drawGreenDistancesMin(rotated_image, adjusted_hole_array, final_streams, ypp, text_size, colors["text"]) # Add this line
			drawGreenDistancesMax(rotated_image, adjusted_hole_array, final_fairways, ypp, text_size, colors["text"])
			drawGreenDistancesTree(rotated_image, adjusted_hole_array, final_trees, ypp, text_size*.8, colors["text"])

			# finally, draw arcs on the fairway every 50 yards from the center of the green
			drawGreenDistancesAnyWaypoint(rotated_image, adjusted_hole_array, ypp, 50, text_size*.7, colors["redtext"])

# REmoved below to make red text vs black added to have full length for all holes next to tee box - change to =0 trying to get on all holes
		...
		drawFeatures(bw_green_image, final_fairways, (164, 200, 144), line=-1)
		drawFeatures(bw_green_image, final_tee_boxes, (195, 195, 195),line=-1)
		drawFeatures(bw_green_image, final_water_hazards, (101,252,239),line=-1)
		drawFeatures(bw_green_image, final_woods, (180,180,180),line=-1)
		drawFeatures(bw_green_image, final_green_array, (188, 230, 164),line=-1) #line to demark green edge in B G R format was 2 not -1
